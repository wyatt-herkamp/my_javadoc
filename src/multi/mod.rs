use std::sync::Arc;
use actix_web::{get, HttpResponse, Resource, Route, routes, web};
...
pub fn register_web(service: &mut ServiceConfig) {
    service.service(web::resource(["/{repository}/{project}/{version}/{file:.*}", "/{repository}/{project}/{version}/"]).name("get_javadoc").route(web::get().to(get_javadoc)));
}
...
pub async fn get_javadoc(requests: web::Data<Sender<ProjectRequest>>, request: web::Path<Request>, repositories: web::Data<Vec<Arc<Repository>>>) -> actix_web::Result<HttpResponse> {
    let repository: Arc<Repository> = repositories.iter().find(|repository| repository.name == request.repository).ok_or(actix_web::error::ErrorNotFound("Repository not found"))?.clone();
